...
var sliderBX = {
  ...
  initSlider: function (device) {
    if (typeof this.slider.getCurrentSlide === "function")
      this.slider.destroySlider();
    this.wrap.html(this.dom);
    if (device !== "desktop")
      this.slider = $(this.settings.slider).bxSlider(new this.Global(device));
  },
  resize: function () {
    this.settings.winWidth = window.innerWidth;
    if (
      this.settings.winWidth <= this.settings.mobile &&
      this.settings.key !== 1
    ) {
      this.settings.key = 1;
      this.initSlider("lite");
    } else if (
      this.settings.winWidth > this.settings.liteDesctop &&
      this.settings.key !== 0
    ) {
      this.settings.key = 0;
...
function glareFastMove() {
  $(window).on("mousemove", function (event) {
    ...
    layer.css({
      transform: "translate(" + left * 20 + "%" + ", " + top * 20 + "%",
    });
  });
}
